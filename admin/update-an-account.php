...
$stmt1 = $mysqli->prepare("SELECT user_details.gender, user_details.firstname, user_details.surname, user_details.studentnumber, user_signin.email, user_details.dateofbirth, user_details.phonenumber, user_details.address1, user_details.address2, user_details.town, user_details.city, user_details.country, user_details.postcode  FROM user_siginin LEFT JOIN user_details ON user_signin.userid=user_details.userid WHERE user_sigin.userid = ? LIMIT 1");
$stmt1->bind_param('i', $idToUpdate);
$stmt1->execute();
$stmt1->store_result();
$stmt1->bind_result($gender, $firstname, $surname, $studentnumber, $email, $dateofbirth, $phonenumber, $address1, $address2, $town, $city, $country, $postcode);
$stmt1->fetch();
$stmt1->close();
