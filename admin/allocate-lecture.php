...
<div class="preloader"></div>
	...
	$stmt1 = $mysqli->query("SELECT user_signin.userid, user_details.studentno, user_details.firstname, user_details.surname FROM user_signin LEFT JOIN user_details ON user_signin.userid=user_details.userid WHERE user_signin.userid NOT IN (SELECT DISTINCT(user_lecture.userid) FROM user_lecture WHERE user_lecture.lectureid = '$lectureToAllocate') AND user_signin.account_type = 'student'");
	...
	$stmt2 = $mysqli->query("SELECT user_signin.userid, user_details.studentno, user_details.firstname, user_details.surname FROM user_signin LEFT JOIN user_details ON user_signin.userid=user_details.userid WHERE user_signin.userid IN (SELECT DISTINCT(user_lecture.userid) FROM user_lecture WHERE user_lecture.lectureid = '$lectureToAllocate') AND user_signin.account_type = 'student'");
