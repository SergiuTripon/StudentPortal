...
if (isset($_GET["id"])) {
	...
	$stmt1 = $mysqli->prepare("SELECT s.account_type, s.email, d.firstname, d.surname, d.gender, d.studentno, d.degree, f.fee_amount, d.nationality, DATE_FORMAT(d.dateofbirth,'%d/%m/%Y') as dateofbirth, d.phonenumber, d.address1, d.address2, d.town, d.city, d.country, d.postcode FROM user_signin s LEFT JOIN user_detail d ON s.userid=d.userid LEFT JOIN user_fee f ON s.userid=f.userid WHERE s.userid=? LIMIT 1");
...
<body>
	...
	<p id="error5" class="feedback-danger text-center"></p>
    ...
    <a id="admin-update-account-submit" class="btn btn-primary btn-lg btn-load">Update account</a>
    </div>
