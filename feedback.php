...
<div class="preloader"></div>
	...
	$stmt1 = $mysqli->query("SELECT DISTINCT system_modules.module_name, user_feedback.feedback_subject, user_feedback.feedback_body,  DATE_FORMAT(user_feedback_lookup.created_on,'%d %b %y %H:%i') as created_on FROM user_feedback_lookup LEFT JOIN system_modules ON user_feedback_lookup.moduleid = system_modules.moduleid LEFT JOIN user_feedback ON user_feedback_lookup.feedbackid = user_feedback.feedbackid WHERE feedback_from = '$session_userid'");
	...
	$stmt1 = $mysqli->query("SELECT DISTINCT user_feedback_lookup.feedbackid, user_details.firstname, user_details.surname, system_modules.module_name,user_feedback.feedback_subject, user_feedback.feedback_body FROM user_feedback_lookup LEFT JOIN user_details ON user_feedback_lookup.feedback_from=user_details.userid LEFT JOIN system_modules ON user_feedback_lookup.moduleid=system_modules.moduleid LEFT JOIN user_feedback ON user_feedback_lookup.feedbackid=user_feedback.feedbackid WHERE isApproved = 0 AND isRead = 0");
