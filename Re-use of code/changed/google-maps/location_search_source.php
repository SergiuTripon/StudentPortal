...
$query = sprintf("SELECT marker_name, marker_notes, marker_lat, marker_long, marker_category, (3959 * acos(cos(radians('%s')) * cos(radians(marker_lat)) * cos(radians(marker_long) - radians('%s')) + sin( radians('%s') ) * sin(radians(marker_lat)))) AS distance FROM system_map_marker HAVING distance < '%s' ORDER BY distance LIMIT 0 , 20",
